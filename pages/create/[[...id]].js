...
import { postData, getData, putData, patchData } from "../../utils/fetchData";
...
const ProductsManager = () => {
  ...
  const handleUploadInput = (e) => {
    ...
    if (files.length === 0) return (err = "File does not exist");
    files.forEach((file) => {
      if (file.size > 1024 * 1024)
        return (err = "The largest file size is 1mb");
  ...
  const handleSubmit = async (e) => {
    ...
    if (auth.user.role !== "admin")
      return dispatch({
        type: "NOTIFY",
        payload: { error: "Authentication is invalid" },
      });
